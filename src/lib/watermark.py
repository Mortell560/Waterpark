...
def add_watermark(image: Image, watermark_text: str, font_size: float = None, color: str = "#CCCCCC") -> Image:
    ...
    for y in range(0, height, y_step):
        for x in range(0, width, x_step):
            draw.text((x, y), watermark_text, fill=color_rgb + (128,), font=font)
...
def add_watermark_to_pdf(pdf_file: bytes, watermark_text: str, font_size: int = None, color: str = "#CCCCCC") -> tuple[FPDF, list[Image]]: # type: ignore
    ...
    for image in images:
        ...
        watermarked_image = add_watermark(image, watermark_text, font_size, color)
...
if __name__ == "__main__":
    ...
    pdf_output, images = add_watermark_to_pdf(pdf_content, watermark_text)
